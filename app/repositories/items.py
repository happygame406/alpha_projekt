...
def _apply_items_filters(stmt, q: str | None, user_id: UUID | None):
    ...
    if q:
        q = q.strip()
        if q:
            stmt = stmt.where(Item.title.like(f'%{q}%'))
    return stmt
...
async def list_items_with_count(session: AsyncSession, q: str | None, user_id: UUID | None, limit: int, offset: int) -> tuple[list[Item], int]:
    data_stmt = select(Item)
    data_stmt = _apply_items_filters(data_stmt, q, user_id)
    ...
    count_stmt = _apply_items_filters(count_stmt, q, user_id)
...
async def list_items_by_user_id(session: AsyncSession, user_id: UUID) -> list[Item]: # удалить позже
