...
async def read_items(session: SessionDep, q: str | None = Query(default=None, description='Поиск по названию'), limit: int = Query(default=20, ge=1, le=100, description='Количество записей на странице'), offset: int = Query(default=0, ge=0, le=100, description='Количество пропущенных записей')):
    items, count = await list_items_with_count(session=session, q=q, user_id=None, limit=limit, offset=offset)
