...
from app.repositories.items import create_item as create_item_repository, list_items_by_user_id, list_items_with_count
...
async def get_user_items(user_id: UUID, session: SessionDep, q: str | None = Query(default=None, description='Поиск по названию'), limit: int = Query(default=20, ge=1, le=100, description='Количество записей на странице'), offset: int = Query(default=0, ge=0, le=100, description='Количество пропущенных записей')):
    user = await get_user(session, user_id)
    if user is None:
        raise HTTPException(status_code=404, detail='User not found')
    items, count = await list_items_with_count(session=session, q=q, user_id=user_id, limit=limit, offset=offset)
